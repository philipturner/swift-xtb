...
public struct Octree {
  ...
  mutating func resizeNodes(
    ...
    func traversalFirstPass(nodeID: Int, levelID: Int) {
      ...
      let x = SIMD8<Float>(0, 1, 0, 1, 0, 1, 0, 1) * 0.5 - 0.25
      let y = SIMD8<Float>(0, 0, 1, 1, 0, 0, 1, 1) * 0.5 - 0.25
      ...
      if node.branchesMask != 0 {
        ...
        for branchID in 0..<8 {
          ...
          if node.branchesMask & branchBit != 0 {
            ...
            var xyz = SIMD3(x[branchID], y[branchID], z[branchID])
            xyz = node.center + xyz * node.spacing
