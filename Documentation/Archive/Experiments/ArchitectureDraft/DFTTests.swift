...
final class DFTTests: XCTestCase {
  ...
  // Scratch space for performing experiments that will be archived.
  func testScratchSpace() throws {
    let resolution: Float = 1e3
    let Z: Int = 2
    
    let spacing = Float(Z) / Float(resolution)
    let spaceCount = Int(10) * Int(resolution)
    let occupations = createShellOccupations(Z: Z)
    let effectiveCharges = createEffectiveCharges(
      Z: Z, occupations: occupations)
    
    let quantumNumbers = QuantumNumbers(n: 1, l: 0, m: 0)
    let correction = Float(1)
